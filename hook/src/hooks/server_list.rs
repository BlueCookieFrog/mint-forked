...
pub unsafe fn init_hooks() -> Result<()> {
    if let Ok(server_name) = &GLOBALS.lock().unwrap().as_ref().unwrap().resolution.server_name {
        GetServerName
            .initialize(
    ...
    if let Ok(server_mods) = &GLOBALS.lock().unwrap().as_ref().unwrap().resolution.server_mods {
...
fn detour_fill_session_setting(
    ...
    unsafe {
        ...
        let name = GLOBALS.lock().unwrap().as_ref().unwrap().meta.to_server_list_string();
